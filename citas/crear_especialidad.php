...
$usuario_id_creador = $_SESSION['usuario_id'] ?? null; // Asignar al usuario que la crea

if (empty($nombre) || empty($usuario_id_creador)) {
    echo json_encode(['success' => false, 'error' => 'El nombre y el ID de usuario son obligatorios.']);
    exit;
}

try {
    $stmt = $conn->prepare("INSERT INTO agenda_especialidades (nombre, usuario_id) VALUES (?, ?)");
